...
export default function AuthModal({ isOpen, onClose, mode }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-auto">
      ...
      <div className="fixed inset-0 bg-black/70 backdrop-blur-xl z-0" onClick={onClose}></div>

      {/* Modal */}
      <div className="relative w-full max-w-lg max-h-screen overflow-y-auto bg-[#0D1117]/95 backdrop-blur-sm border border-white/20 rounded-3xl shadow-2xl z-10 animate-in fade-in zoom-in duration-300">
        {/* Gradient top accent */}
        ...
        <button
          onClick={onClose}
          className="absolute top-6 right-6 w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-all text-slate-400 hover:text-white z-10 border border-white/10"
        >
        ...
        <div className="p-10">
            ...
            {loading ? (
              ...
              <>
                <svg className="w-6 h-6" viewBox="0 0 24 24">
                  ...
                  <path fill={mode === 'signup' ? 'currentColor' : '#EA4335'} d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <span>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-[#0D1117] text-slate-500">ou</span>
            </div>
          </div>
          ...
          <div className="text-center">
            <p className="text-slate-400 text-sm">
              ...
              <button 
                onClick={() => {
                  onClose();
                  // Small delay to allow modal to close before opening the other one
                  setTimeout(() => {
